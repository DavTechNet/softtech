...
{
    QModelIndexList indexes = ui->tableView_listSoft->selectionModel()->selectedIndexes();
    if (indexes.count() != 0) // Seulement si une ligne est selectionnée
    {
        qDebug() << "DeleteSoft";
        ChangeButtonsStatus(false);

        int soft_id = indexes[9].data().toInt();
        tableLoan = Database::RWTableLoan();
        tableSoft = Database::RWTableSoft();
        tableSoft->setEditStrategy(QSqlTableModel::OnManualSubmit);
        tableLoan->setEditStrategy(QSqlTableModel::OnManualSubmit);
        ...
        // Suppression des emprunts du logiciel
        tableLoan->setFilter("id_logiciel='" + QString::number(soft_id) + "'");
        tableLoan->removeRows(0, tableLoan->rowCount());

        // mise a jour des id_logiciel des emprunts
        tableLoan->setFilter("id_logiciel >= " + QString::number(soft_id));
        for (int i = 0; i < tableLoan->rowCount(); i++)
        {
            QModelIndex index = tableLoan->index(i, 4);
            tableLoan->setData(index, QVariant(index.data().toInt() - 1));
        }
